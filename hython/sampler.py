...
class AbstractDownSampler(ABC):
    ...
    @abstractmethod
    def sampling_idx(self, shape: tuple[int], grid: NDArray | xr.DataArray | xr.Dataset) -> SamplerResult:
...
class RegularIntervalDownSampler(AbstractDownSampler):
    ...
    def sampling_idx(self, shape, missing_mask = None, grid = None): # remove missing is a 2D mask
...
class SamplerBuilder(TorchSampler):
    ...
    def initialize(self, shape = None, mask_missing = None, grid = None, torch_dataset = None):
        ...
        self.result = self.method_instance.sampling_idx(shape, self.mask_missing, self.grid)
